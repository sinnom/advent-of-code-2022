...
fn main() {
    let priority_map: HashMap<char, u32> = ('a'..='z').chain('A'..='Z').zip(1..=56).collect();

    let answer: u32 = get_input()
        .split('\n') // Iter of rucksack contents
        .chunks(3)
        .into_iter() // Iter of elf groups
        .filter_map(|chunks| chunks.collect_tuple()) // Filter out groups less than 3 (empty group due to newline at end of input )
